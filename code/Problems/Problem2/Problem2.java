...
public class Problem2 extends java.applet.Applet
{  
   ...
   public static int[] nodesHit = new int[6];
   /*
	...
	if (evt.target instanceof Button) 
        { 
          ...
          {  
             ...
             JOptionPane.showMessageDialog(null, Problem2.hints[Problem2.hintNo]);
             Problem2.hintNo++;
             if(Problem2.hintNo==3){Problem2.hintNo=0;}
...
class InfoText extends TextArea 
{
    ...
    "to city F in the shortest path, by clicking the grey ovals in " +
    "turn.\n");
...
class GraphCanvas extends Canvas implements Runnable 
{
 ...
 w=this.size().width/8;
	...
	node[2]=new Point(6*w, h);   
	node[3]=new Point(4*w, 3*h); 
	node[0]=new Point(w, 7*h);
    node[4]=new Point(5*w, 8*h);   
	node[5]=new Point(7*w, 7*h); 
    ...
    {  j = i;
	  if ( (x-node[i].x)*(x-node[i].x) + 
				...
				(y-node[i].y)*(y-node[i].y) < dist*dist ) 
                {
	              ...
	              hitnode = i;
                  Problem2.nodesHit[Problem2.nodeHits]=hitnode;  
                  //nodesHit[Problem2.nodeHits]=hitnode;
                  
                  //System.out.println(Problem2.nodeHits);
    ...
    public boolean mouseDown(Event evt, int x, int y) 
    {
	  ...
	  // paint the node Yellow! 
         ...
         {  Problem2.nodeHits++;  
	        node1 = hitnode;
            checkPath();
	           if((node[node1] == node[5]) & (correctPath))
               ...
               else if((node[node1] == node[5]) & (!correctPath))
                { //System.out.println("last node"); 
                  checkPath();
   ...
   {
      int[] correctNodesHit = {1,3,2,5,0,0};
      int[] correctNodesHit2 = {0,1,3,2,5,0};
     // System.out.println("user array"); for(int d=0;d<6;d++){System.out.println(Problem2.nodesHit[d]);}
     // System.out.println(numnodes);
      boolean checkNodePath = Arrays.equals(Problem2.nodesHit, correctNodesHit);
      boolean checkNodePath2 = Arrays.equals(Problem2.nodesHit, correctNodesHit2);
