...
public class Problem3 extends java.applet.Applet
{  
   ...
   public static int[] nodesHit = new int[8];
   /*
...
class Buttons extends Panel 
{
    ...
    public void nextExample() // not currently implemented, nowhere to put the method call outside of a loop!
    {
      ...
      // int answer = JOptionPane.showConfirmDialog(NextExampleFrame, "Next Example?");
       int answer = JOptionPane.showConfirmDialog(null, "Do you want to try the next problem?", "Next Problem?",JOptionPane.YES_NO_OPTION);
	...
	if (evt.target instanceof Button) 
        { 
          ...
          {  
             ...
             JOptionPane.showMessageDialog(null, Problem3.hints[Problem3.hintNo]);
             Problem3.hintNo++;
             if(Problem3.hintNo==3){Problem3.hintNo=0;}
...
class InfoText extends TextArea 
{
    ...
    "to city H in the shortest path, by clicking the grey ovals in " +
    "turn.\n");
...
class GraphCanvas extends Canvas implements Runnable 
{
 ...
 w=this.size().width/9;
	h=this.size().height/9;     
    node[0]=new Point(w, 7*h);   
	node[1]=new Point(2*w, h);   
	node[2]=new Point(3*w, 1*h);   
	node[3]=new Point(3*w, 7*h); 
	node[4]=new Point(5*w, h);
    node[5]=new Point(6*w, 6*h);
    node[6]=new Point(7*w, 3*h);
    node[7]=new Point(8*w, 7*h); 
    ...
    {  j = i;
	  if ( (x-node[i].x)*(x-node[i].x) + 
				...
				(y-node[i].y)*(y-node[i].y) < dist*dist ) 
                {
	              ...
	              hitnode = i;
                  Problem3.nodesHit[Problem3.nodeHits]=hitnode;  
                  //nodesHit[Problem3.nodeHits]=hitnode;
                  
                  //System.out.println(Problem3.nodeHits);
    ...
    public boolean mouseDown(Event evt, int x, int y) 
    {
	  ...
	  // paint the node Yellow! 
         ...
         {  Problem3.nodeHits++;  
	        node1 = hitnode;
            checkPath();
	           if((node[node1] == node[7]) & (correctPath))
               ...
               else if((node[node1] == node[7]) & (!correctPath))
                { //System.out.println("last node"); 
                  checkPath();
   ...
   {
      int[] correctNodesHit = {3,5,7,0,0,0,0,0};
      int[] correctNodesHit2 = {0,3,5,7,0,0,0,0};
      //System.out.println("user array"); for(int d=0;d<8;d++){System.out.println(Problem3.nodesHit[d]);}
      //System.out.println(numnodes);
      boolean checkNodePath = Arrays.equals(Problem3.nodesHit, correctNodesHit);
      boolean checkNodePath2 = Arrays.equals(Problem3.nodesHit, correctNodesHit2);
